...
/*
  A "type" - это тип (форма) фигуры тетриса (не включая ориентацию)
 */
...
typedef struct GameStruct {
  ...
  int level;
  /*
    Falling block - падающий в данный момент
    Next block -следующий падающий блок
    Stored is the block that you can swap out (в холдере).
   */
  Block falling;
  Block next;
  Block stored;
  /*
    Кол-во game ticks до падения след блока
   */
